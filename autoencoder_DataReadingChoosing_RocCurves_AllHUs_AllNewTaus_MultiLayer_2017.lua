...
print('\n\n @ @ @ @ @ @ START @ @ @ @ @ @ @ ');
print('Torch program that creates a simple AutoEncoder-style neural network, where output=input, and applies it to a gene-functional relationship Gene Ontology matrix');
print('author Davide Chicco <davide.chicco@gmail.com>');
timeStart = os.time();
print(os.date("%c", os.time()));
istante = os.date("%c", os.time());
print('version with 10% in the input matrix and ROC curves when the confusion matrix threshold varies');
...
repeat
   ...
   io.flush()
   hl_max=io.read()
   hl_max = tonumber(hl_max)
until hl_max >= 0 and hl_max < 9

print('You chose '..hl_max..' hidden layer(s)');
...
fileAucs = {}
fileAucs[hl_counter] ='data/_aucs_autoencoder_'..datasetName..'_'..hl_counter..'hl_'..istante..'.txt';
printAucs = true;
fAucs = {};
if printAucs then fAucs[hl_counter],err = io.open(fileAucs[hl_counter],"w"); end
    ...
    --   for h=302, sampleNumber, 1 do -- ONE SHOT
    ...
    -- 		    -- print('\t\t\t-- testing --');
	   ...
	   -- print('\t-- Cycle for every possible value of threshold [END]');
	    -- <<</// Ciclo per ogni possibile valore di threshold >>> --
	      ...
	      require './lib/sort_two_arrays_from_first';
	      newACrateArray = {}
	      newAPrateArray = {}
