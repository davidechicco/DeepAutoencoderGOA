...
-- for h=511, 512, step do -- ONE SHOT
	...
	ARrateArray = torch.Tensor(inputs);
	minErrorRate = 100;
	minErrorRateIndex = 0;
	bestThreshold = -1;
...
-- 		    -- print('\t\t\t-- testing --');
	...
	if printAPlist then
	  ...
	  ARrate = globalAR / (globalAR + globalAC);
	  ...
	  ARrateArray[q] = ARrate;-- ATTENZIONE: APrateArray va da 1 a #
	  
	  -- Best threshold selection
	  if (ARrate + APrate) < minErrorRate then
	      minErrorRate = (ARrate + APrate);
	      minErrorRateIndex = q;
	      bestThreshold = (ARrate + APrate)
	  end
	end 
	...
	-- <<</// Ciclo per ogni possibile valore di threshold >>> --
	  ...
	  require './funzioni/sort_two_arrays_from_first'
	    ...
	    -- print('the AUC is ' .. area ..'% when HUs='..HUs .. '\n');
	     ...
	     print('best threshold here is ' .. bestThreshold .. '\n');
