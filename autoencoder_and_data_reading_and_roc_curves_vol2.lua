...
ACrateArray = torch.Tensor(maxThresholdArray);
APrateArray = torch.Tensor(maxThresholdArray);
start = 1;
decremento = 0.01;
for i=0, maxThresholdArray-1 do
  thresholdArray[i] = start;
  start = start-decremento;
end
for i=1, maxThresholdArray do
  ACrateArray[i] = 0;
  APrateArray[i] = 0;
end
...
if printAPlist then
  ...
  ACrateArray[q+1] = ACrate; -- ATTENZIONE: ACrateArray va da 1 a #
  APrateArray[q+1] = APrate;-- ATTENZIONE: APrateArray va da 1 a #
...
ACrateArraySorted = {};
for q=1, #ACrateArray do
  ACrateArraySorted[q] = ACrateArray[q];
end
table.sort(ACrateArraySorted);

for i=1, #ACrateArray do
    for j=1, #ACrateArraySorted do
      if ACrateArray[i]== ACrateArraySorted[j] then print('The element '.. ACrateArray[i] ..' was '..i..'th in the former table and now it is ' .. j ..'th \n');
      end
      j = j+1;
    end
    i = i+1;
...
-- Sistemiamo gli ultimi due ACrate ed APrate che rimangono a zero: VERIFICARE!
ACrateArray[maxThresholdArray] = ACrateArray[maxThresholdArray-1];
APrateArray[maxThresholdArray] = APrateArray[maxThresholdArray-1];

print('\nACrateArray['..#thresholdArray..'] ' .. ACrateArray[#thresholdArray]);
print('\nAPrateArray['..#thresholdArray..'] ' .. APrateArray[#thresholdArray]);

 area = 0;
 -- print ACrate and APrate into the file
for q=1, #thresholdArray do
  if printRoc then
    ACrateArray[q] = round(ACrateArray[q], 3);
    APrateArray[q] = round(APrateArray[q], 3);
    -- fRoc:write(ACrate .. '\t' .. APrate ..'\t' ..  threshold .. '\n');
     fRoc:write(ACrateArray[q] .. '\t' .. APrateArray[q] ..'\n');
  end  
  
  if q>=2 then
    area = area + ((APrateArray[q] - APrateArray[q-1]) * (ACrateArray[q-1] + ACrateArray[q]))/2;
  end 
...
gnuplot.plot(APrateArray, ACrateArray, '-');
