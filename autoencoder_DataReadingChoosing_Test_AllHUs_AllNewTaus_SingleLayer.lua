...
fileAucs = 'data/_aucs_autoencoder_'..datasetName..'.txt';
printAucs = false;
		    ...
		    for i=1,inputs do
		      ...
		      if x[i]==0 and array[i]> threshold then
			    if printAPlist then
			      ...
			      APlistValueArray[geneProfileChosen*u] = array[i];
			      APlistGeneOidArray[geneProfileChosen*u] = genes[geneProfileChosen];
			      APlistFeatureOidArray[geneProfileChosen*u] = features[i-1]
			    end
			    localAP=localAP+1;
			    globalAP=globalAP+1;
	...
	if printAPlist then
	  ...
	  f2:close();
 	  ...
 	  require './funzioni/sort_two_arrays_from_first'
	      ...
	      APlistG = {}
	      APlistF = {}
	      APListV, APlistG, APlistF =  sort_three_arrays_from_first(APlistValueArray, APlistGeneOidArray,  APlistFeatureOidArray)
