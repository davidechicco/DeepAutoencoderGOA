...
function AucComputer(tens1, tens2, size, printRoc, fileRoc)
	...
	for q=1, #thresholdArray do
	  ...
	  if q>=2 then
	    area = area + ((tens2[q] - tens2[q-1]) * (tens1[q-1] + tens1[q]))/2;
	  end 
...
-- 		    -- print('\t\t\t-- testing --');
	...
	area = AucComputer(ACrateArray, APrateArray, #thresholdArray, printRoc, fileRoc)
	-- areaArray[h] = area; -- ONE SHOT
	print('the AUC is ' .. area ..'% when HUs='..HUs .. '\n');
	if printAucs then  fAucs:write(' ' ..HUs .. ' ' .. area .. ' \n'); end
